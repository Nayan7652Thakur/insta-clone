...
const EditProfile = () => {
  ...
  const [input, setInput] = useState({
    ...
    bio: user?.bio,
    gender: user?.gender || 'male' // Fallback gender value if not provided
  })
  ...
  const editProfileHandler = async () => {
    ...
    formData.append("gender", input.gender) // Correctly appending gender
    ...
    try {
      ...
      const res = await axios.post('http://localhost:8000/api/v2/user/profile/edit', formData, { // Fixed typo in the URL
        headers: {
      ...
      <section className='flex flex-col gap-6 w-full'>
        ...
        <div className='flex items-center justify-between bg-gray-100 rounded-xl p-4'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              <h1 className='font-bold text-sm'>{user?.userName || 'Anonymous'}</h1>
        ...
        <div className='flex justify-end'>
          {loading ? (
            ...
            <Button onClick={editProfileHandler} className='w-fit bg-[#0095F6] hover:bg-[#0b67a4]'>
              Submit
            </Button>
          )}
        </div>
